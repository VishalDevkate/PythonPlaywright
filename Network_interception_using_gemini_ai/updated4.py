...
def run(playwright):
    ...
    # 🌟 NEW STRATEGIC PLACEMENT FOR DEMO 1 VERIFICATION:
    # Now that the page has loaded the landing page and processed the login form,
    # check if any auxiliary style images/logos were successfully blocked.
    print(f"\n--- Demonstration 1: Image Blocking Check ---")
    print(f"Blocked Image Requests Count: {len(failed_image_urls)}")

    # If the app still hasn't requested any images yet, we force an image block check
    # by unrouting right here so it won't crash our checkout flow below.
    ...
    # The grid will now easily load and display since images are completely unblocked
