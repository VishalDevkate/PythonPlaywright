...
def run(playwright):
    ...
    # ------------------------------------------------------------------
    # REGISTER ALL INTERCEPTORS BEFORE NAVIGATING (Crucial Fix)
    # ------------------------------------------------------------------
    ...
    # FIX: Catch and mock the catalog fetching routine globally
    def mock_product_catalog(route):
        catalog_mock = {
            "data": [
                {
                    ...
                    "productPrice": 31500,
                    "productDescription": "Zara Coat for testing",
                    "productImage": "https://rahulshettyacademy.com/api/ecom/uploads/productImage_1650649085934.jpeg",
                    "productRating": "0",
                    "productStatus": True,
                    "productFor": "women",
        ...
        route.fulfill(
            status=200,
            content_type="application/json",
            body=json.dumps(catalog_mock),
        )

    page.route("**/api/ecom/product/get-all-products", mock_product_catalog)
    ...
    # 1. Now safely browse to the site with the network interception rules running
    page.goto("https://rahulshettyacademy.com/client/")
    page.wait_for_load_state("domcontentloaded")

    # DEMO 2 EXECUTION: Trigger Login API Call
    print(f"--- Demonstration 2: API Mocking (Login) ---")
    ...
    # DEMO 1 VERIFICATION: Inspect blocked image metrics
    print(f"\n--- Demonstration 1: Image Blocking Check ---")
    print(f"Blocked Image Requests Count: {len(failed_image_urls)}")

    # Lift image blocker rules so the dynamic checkout asset elements can render
    page.unroute("**/*.{png,jpg,jpeg,svg}", image_blocker_handler)
    print("🔄 Info: Lifted image blocking rules to guarantee checkout buttons render safely.")

    # DEMO 3 EXECUTION: Order Placement Modification
    print(f"\n--- Demonstration 3: Response Modification (Orders) ---")

    # Wait for catalog UI component to arrive safely
