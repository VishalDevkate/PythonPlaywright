...
def run(playwright):
    ...
    # ------------------------------------------------------------------
    # FIX: Mock Dashboard Catalog to prevent empty dashboard hangs
    # ------------------------------------------------------------------
    ...
    # Tear down image blocker so product images don't freeze the action elements
    page.unroute("**/*.{png,jpg,jpeg,svg}", image_blocker_handler)
    print("🔄 Info: Cleaned up image blocker routing to ensure the dashboard grid loads.")

    # DEMO 3 EXECUTION: Order Placement Modification
    print(f"\n--- Demonstration 3: Response Modification (Orders) ---")

    # The grid will now cleanly render our mock item list
