...
def run(playwright):
    ...
    # ------------------------------------------------------------------
    # USE CASE 1: Abort/Block Requests (Modified to let core app design assets pass)
    # ------------------------------------------------------------------
    def block_images(route):
        # If the image belongs to the product catalog, we let it pass so the dashboard grid loads
        if "product" in route.request.url.lower():
            route.continue_()
        else:
    ...
    # The grid will now bypass the absolute freeze and become visible
    page.locator(".card-body").first.wait_for(state="visible", timeout=10000)
    ...
    # DEMO 1 VERIFICATION
    # ------------------------------------------------------------------
    print(f"\n--- Demonstration 1: Image Blocking ---")
    print(f"Blocked Image Requests Count: {len(failed_image_urls)}")
    assert len(failed_image_urls) > 0, "Failed to block images!"
    print("✅ Verified: Non-catalog background images successfully blocked via interceptor.")
